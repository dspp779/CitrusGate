...
直接啟動會呼叫 macOS 的 `open -n`，依 `.exe` 的預設開啟 App 建立新的 Cyder instance。已核對可直接帶入登入參數的遊戲如下：
...
天堂系列、新龍之谷與 CSO 尚無可核對的命令列登入格式，因此 App 不會誤傳楓之谷參數；會先複製 OTP，再以 `open -n` 開啟使用者指定的主程式。
