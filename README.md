...
直接啟動會向 macOS 查詢 `MapleStory.exe` 的預設開啟 App，再以獨立參數傳入下列內容（路徑即使包含空白也不需自行加引號）：

```text
--launch-exe /path/to/MapleStory.exe -- tw.login.maplestory.beanfun.com 8484 BeanFun <ServiceAccountID> <OTP>
```
