...
struct GameDefinition: Hashable {
    let id: String
    let name: String
...
enum LaunchCommandBuilder {
    ...
    static func nexonWineCommand(
        ...
        wine --bottle maplestory --wait-children --enable-alt-loader macdrv --workdir \(shellQuote(workdir)) \(shellQuote(executablePath)) \(args)
        """
    }
